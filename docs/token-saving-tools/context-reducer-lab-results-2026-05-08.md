...
| `repo_map` | `project_file_index` | `required_path_overlay` | oracle upper bound only | 4/4 cases still reached 100% coverage, but the row is now explicitly marked `oracle_upper_bound` because it uses `required_paths`. Real variants do not use validation labels: `expanded_prompt_component_map` is the best current signal at 75.0% average coverage, while prompt-anchor/search-seeded/hybrid variants remain shadow-only and inconsistent. |
...
| `process` | `process_table_compact` | `top_resource_plus_named` / filtered-process routing | shadow only | Extended live+fixture evidence covers `Get-Process`, targeted `Get-Process`, `tasklist`, `ps`, CIM/WMI, and filtered pipelines. The focused Codex/Cargo/Rust filter is useful for build-collision orientation but fails generic process targets; the Codex shadow renderer now keeps top rows plus named Codex/Cargo/Rust/PowerShell rows and filtered process pipelines route to `process_table_compact`. No process reducer is direct-replacement safe. |
