...
| Blackboard coordination | `34c8e537c0`, `21700e4565`, integrated by `be3880b3b5`; owner-crate extraction `3677c3ddc5` | `codex-rs/blackboard`, thin adapter in `codex-rs/core/src/session/blackboard.rs` | `codex-blackboard`, then MultiAgentV2 canary if core session wiring changes |
...
| TUI history-cell fork UX behavior | split-module port in current refactor pass | `codex-rs/tui/src/history_cell/*` | Targeted `codex-tui` history-cell snapshot/unit canary |
| Turn-diff tracking ownership | owner-crate extraction `3677c3ddc5` | `codex-rs/turn-diff`, thin adapters in `codex-rs/core/src/turn_diff_tracker.rs` and tool event handling | `codex-turn-diff`; core compile canary if event plumbing changes |
| Cognos operation ownership | owner-crate extraction `3677c3ddc5` | `codex-rs/cognos-ops`, thin adapter in `codex-rs/core/src/tools/handlers/cognos_ops.rs` | `codex-cognos-ops`; targeted core tool canary if runtime invocation changes |
...
- `codex-rs/tui/src/history_cell/*`: upstream's split history-cell layout is the owner for transcript rendering behavior; local fork UX deltas should be ported into leaf modules instead of reintroducing `history_cell.rs`.
