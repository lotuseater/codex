...
pub struct McpServerStatus {
    pub name: String,
    pub tools: HashMap<String, codex_protocol::mcp::Tool>,
...
pub struct McpServerDisplayConfig {
    pub transport: McpServerDisplayTransport,
    pub enabled: bool,
    pub disabled_reason: Option<String>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum McpServerDisplayTransport {
    Stdio {
        command: String,
        args: Vec<String>,
...
impl McpToolCallCell {
    ...
    fn render_content_block(block: &serde_json::Value, width: usize) -> String {
        ...
        let Some(content_type) = block.get("type").and_then(serde_json::Value::as_str) else {
            return fallback();
        };

        match content_type {
            "text" => match block.get("text").and_then(serde_json::Value::as_str) {
                Some(text) => format_and_truncate_tool_result(text, TOOL_CALL_MAX_LINES, width),
                None => fallback(),
            },
            "image" => "<image content>".to_string(),
            "audio" => "<audio content>".to_string(),
            "resource" => block
                .get("resource")
                .and_then(|resource| resource.get("uri"))
                .and_then(serde_json::Value::as_str)
                .map(|uri| format!("embedded resource: {uri}"))
...
pub fn new_mcp_tools_output(
    effective_servers: &HashMap<String, McpServerDisplayConfig>,
...
#[cfg(test)]
fn qualified_mcp_tool_name_prefix(server: &str) -> String {
    format!("mcp__{}__", server.replace('-', "_"))
}

/// Build the `/mcp` history cell from server `McpServerStatus` responses.
///
/// The server list merges the app-server status response with the effective
/// display config so configured servers without tools are still visible.
///
...
pub fn new_mcp_tools_output_from_statuses(
    effective_servers: &HashMap<String, McpServerDisplayConfig>,
    ...
    let statuses_by_name: HashMap<&str, &McpServerStatus> = statuses
        .iter()
        .map(|status| (status.name.as_str(), *status))
        .collect();
    let mut server_names: Vec<String> = effective_servers.keys().cloned().collect();
    server_names.extend(statuses.iter().map(|status| status.name.clone()));
    server_names.sort();
    server_names.dedup();

    for server in server_names {
        let cfg = effective_servers.get(server.as_str());
...
#[derive(Debug)]
pub struct McpInventoryLoadingCell {
    start_time: Instant,
    animations_enabled: bool,
}
...
pub fn new_mcp_inventory_loading(animations_enabled: bool) -> McpInventoryLoadingCell {
