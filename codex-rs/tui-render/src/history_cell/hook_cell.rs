...
impl HookCell {
    ...
    pub fn take_completed_persistent_runs(&mut self) -> Option<Self> {
    ...
    pub fn has_visible_running_run(&self) -> bool {
    ...
    pub fn start_run(&mut self, run: HookRunSummary) {
        let now = Instant::now();
    ...
    pub fn complete_run(&mut self, run: HookRunSummary) -> bool {
    ...
    pub fn add_completed_run(&mut self, run: HookRunSummary) {
...
pub fn new_active_hook_cell(run: HookRunSummary, animations_enabled: bool) -> HookCell {
    HookCell::new_active(run, animations_enabled)
}

pub fn new_completed_hook_cell(run: HookRunSummary, animations_enabled: bool) -> HookCell {
...
mod tests {
    ...
    #[test]
    fn persistence_policy_drops_only_quiet_tool_hook_failures() {
        assert!(!completed_hook_should_persist(
