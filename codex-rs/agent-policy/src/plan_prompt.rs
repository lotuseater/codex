pub const MAIN_AGENT_PLAN_DELEGATION_PROMPT: &str = "Every main-agent task plan prompt must inject a delegation decision: state what to delegate to subagents when delegation is useful, or state that the work stays local and why. On a first task plan, or when loop mode is planning a continuation, prefer reusing or creating up to three persistent high-capability helper agents for nontrivial or context-drift-prone work, and delegate even simple or short subtasks when that keeps root context smaller. Ask subagents for a short summary or short result only when the main agent needs that handoff to integrate, verify, or review their work.";
...
Every main-agent task plan prompt must inject a delegation decision: state what to delegate to subagents when delegation is useful, or state that the work stays local and why. On a first task plan, or when loop mode is planning a continuation, prefer reusing or creating up to three persistent high-capability helper agents for nontrivial or context-drift-prone work, and delegate even simple or short subtasks when that keeps root context smaller. Ask subagents for a short summary or short result only when the main agent needs that handoff to integrate, verify, or review their work.

On a first task plan, or when loop mode is planning a continuation, bias toward a reusable helper pool for nontrivial or context-drift-prone work: reuse existing suitable helpers first, otherwise create up to three persistent high-capability helpers using the best available model and reasoning effort. Keep the pool around through the session and delegate extensively, including simple or short subtasks, when the work is bounded and the handoff can stay compact. A useful default specialization is: code/context probe, verification or test triage, and design/review or prompt-reduction impact. State any local-only exception explicitly.

Only the main/root agent spawns helpers. Subagents must not recursively spawn more helpers; they should use the assigned tools and return a bounded handoff to root.
...
Do not spawn more agents. Only the main/root agent creates or reuses helper agents; if you need more context or another specialty, send a concise request or handoff back to root.
