...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder =
        codex_core_test_runtime::test_codex::test_codex().with_config(move |config| {
            config
                .features
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = codex_core_test_runtime::test_codex::test_codex().with_config(|config| {
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = codex_core_test_runtime::test_codex::test_codex();
    let test = builder
        .with_config(|config| {
