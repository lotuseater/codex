    ...
    fn apply(&mut self, edit: &ConfigEdit) -> anyhow::Result<bool> {
        match edit {
            ...
            ConfigEdit::SetContextBudgetMode { mode } => {
                Ok(self
                    .write_profile_value(&["context_budget_mode"], Some(value(mode.to_string()))))
            }
            ...
            ConfigEdit::SetProjectTrustLevel { path, level } => {
                // Preserve the explicit project-table shape used by the loader.
...
fn set_project_trust_level_inner(
...

    {
        ...
        {
            let mut projects_table = TomlTable::new();
            projects_table.set_implicit(true);

            if let Some(inline_table) = existing_projects
                .as_ref()
                .and_then(|item| item.as_inline_table())
            {
                for (key, value) in inline_table.iter() {
                    if let Some(inner_table) = value.as_inline_table() {
                        projects_table
                            .insert(key, TomlItem::Table(inner_table.clone().into_table()));
                    }
                }
            }

            root.insert("projects", TomlItem::Table(projects_table));
        }
    }
