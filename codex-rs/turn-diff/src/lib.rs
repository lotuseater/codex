...
pub struct TurnDiffTracker {
    ...
    /// Fallback display root for non-git worktrees.
...
impl TurnDiffTracker {
    ...
    pub fn track_delta(&mut self, changes: &[CommittedFileChange]) -> bool {
        for change in changes {
            match change {
                CommittedFileChange::Add {
                    ...
                    let content = overwritten_content.as_deref();
                    self.track_delta_baseline(path, path, content);
                }
                CommittedFileChange::Delete { path, content } => {
                    self.track_delta_baseline(path, path, Some(content));
                }
                CommittedFileChange::Update {
                    ...
                    if let Some(dest) = move_path {
                        if let Some(overwritten_content) = overwritten_move_content {
                            self.track_delta_baseline(path, path, Some(old_content));
                            self.track_delta_baseline(dest, dest, Some(overwritten_content));
                        } else {
                            self.track_delta_baseline(path, dest, Some(old_content));
                        }
                    } else {
                        self.track_delta_baseline(path, path, Some(old_content));
                    }
    ...
    pub fn on_patch_begin(&mut self, changes: &HashMap<PathBuf, TrackedFileChange>) {
        for (path, change) in changes.iter() {
            let uuid_filename = self.ensure_path_tracked(path);

            // Track rename/move in current mapping if provided in an Update.
            if let TrackedFileChange::Update {
                move_path: Some(dest),
            } = change
            {
                if dest.exists() {
                    self.ensure_path_tracked(dest);
                    continue;
                }
    ...
    fn ensure_path_tracked(&mut self, path: &Path) -> String {
        ...
        // If the file exists on disk now, snapshot as baseline; otherwise use
        // a missing-file baseline so later content renders as an addition.
    ...
    fn track_delta_baseline(
        &mut self,
        baseline_path: &Path,
        current_path: &Path,
        content: Option<&str>,
    ) {
        if self.external_to_temp_name.contains_key(baseline_path)
            || self.external_to_temp_name.contains_key(current_path)
        {
            return;
        }

        let internal = Uuid::new_v4().to_string();
        let (content, oid) = match content {
            Some(content) => {
                let content = content.as_bytes().to_vec();
                let oid = git_blob_sha1_hex_bytes(&content);
                (content, oid)
            }
            None => (Vec::new(), ZERO_OID.to_string()),
        };

        self.external_to_temp_name
            .insert(current_path.to_path_buf(), internal.clone());
        self.temp_name_to_current_path
            .insert(internal.clone(), current_path.to_path_buf());
    ...
    /// Return a display string for `path` relative to its git root, or the configured display root if no git root is found.
    fn relative_to_git_root_str(&mut self, path: &Path) -> String {
        ...
        } else if let Some(root) = self.display_root.as_ref()
