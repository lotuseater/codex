...
/// The app-server-facing analytics reducer.
///
...
impl codex_analytics::AnalyticsReducer for AppServerReducer {
    async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEvent>) {
        ...
        // app-server emit byte-identical events.
        if let AnalyticsFact::Custom(custom) = &input
            && is_unconditional_custom(custom)
        {
            self.custom.ingest(input, out).await;
            return;
        }

        let mut reqs: Vec<TrackEventRequest> = Vec::new();
        match input {
            AnalyticsFact::AppServer(value) => {
            ...
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(input)) => {
                self.ingest_compaction(*input, &mut reqs);
            }
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(input)) => {
                self.ingest_guardian_review(*input, &mut reqs);
            }
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnResolvedConfig(input)) => {
                self.ingest_turn_resolved_config(*input, &mut reqs).await;
            }
        ...
        for req in reqs {
            let isolated = req.should_send_in_isolated_request();
            out.push(TrackEvent::from_serializable(&req, isolated));
        }
    }
}
...
impl AppServerReducer {
    pub(crate) async fn ingest_app_server(
        &mut self,
        fact: AppServerFact,
        ...
        match fact {
            AppServerFact::Initialize {
                ...
                self.ingest_initialize(connection_id, params, product_client_id, rpc_transport);
            }
            ...
            AppServerFact::Notification(notification) => {
    ...
    pub(crate) fn ingest_guardian_review(
        &mut self,
    ...
    pub(crate) async fn ingest_turn_resolved_config(
        &mut self,
    ...
    pub(crate) async fn ingest_turn_token_usage(
        &mut self,
    ...
    pub(crate) fn ingest_compaction(
        &mut self,
...
fn codex_turn_event_params(
    ...
    CodexTurnEventParams {
        ...
        // Upstream added an optional error subreason dimension; the turn-state
        // fact carries no subreason yet, so emit None.
