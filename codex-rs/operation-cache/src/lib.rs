...
pub async fn lookup(tool_name: &str, tool_input: &Value, cwd: &Path) -> Option<OperationCacheHit> {
    if !tool_is_cacheable(tool_name) {
        return None;
    }
    let config = OperationCacheConfig::from_env()?;
    let scope = ProjectCacheScope::from_cwd(cwd);
    let input = serde_json::json!({
        "event": cache_event(tool_name, tool_input, cwd, &scope),
    });
...
pub async fn store(tool_name: &str, tool_input: &Value, tool_response: &Value, cwd: &Path) {
    if !tool_is_cacheable(tool_name) {
        return;
    }
    ...
    let input = serde_json::json!({
        "event": cache_event(tool_name, tool_input, cwd, &scope),
        "output": output_text(tool_response),
        "success": true,
    });
...
pub fn tool_is_cacheable(tool_name: &str) -> bool {
    !tool_name.starts_with("dab_")
}
...
mod tests {
    ...
    #[test]
    fn tool_cache_skips_live_dab_tools() {
        assert!(!tool_is_cacheable("dab_screenshot"));
        assert!(!tool_is_cacheable("dab_visual_scan"));
        assert!(!tool_is_cacheable("dab_find_window"));
        assert!(tool_is_cacheable("automation_harness_detect"));
        assert!(tool_is_cacheable("shell"));
    }
}
