    ...
    pub fn new(
        ...
        Self {
            queue: (analytics_enabled != Some(false)).then(|| {
                AnalyticsEventsQueue::new(Arc::clone(&auth_manager), destination, reducer)
            }),
        }
    }
...
fn capture_track_events_request(
    destination: &AnalyticsEventsDestination,
    // fork-local: the fork's decoupled pipeline produces a `serde_json::Value`
    // payload (via `TrackEvent::into_body`), not upstream's concrete
    // `TrackEventsRequest`. `append_payload` serializes any `Serialize` value.
