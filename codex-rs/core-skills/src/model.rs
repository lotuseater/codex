...
/// Fork-compat alias: upstream renamed `HostLoadedSkills` -> `HostSkillsSnapshot`
/// (same shape/behavior). Kept so the fork-era path `codex_core_skills::HostLoadedSkills`
/// still resolves for any out-of-tree consumer; prefer `HostSkillsSnapshot` in new code.
pub type HostLoadedSkills = HostSkillsSnapshot;
