...
impl UnifiedExecProcessManager {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        // `ExecApprovalRequest::sandbox_cwd` borrows a `&Path`; `request.sandbox_cwd`
        // is now a `PathUri`, so resolve it to an owned native path first (mirrors the
        // `to_abs_path()`/`ForeignPath` boundary handling used elsewhere in this file).
        let sandbox_cwd =
            request
                .sandbox_cwd
                .to_abs_path()
                .map_err(|_| UnifiedExecError::ForeignPath {
                    path: request.sandbox_cwd.clone(),
                })?;
