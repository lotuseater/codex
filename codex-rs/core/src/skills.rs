...
pub type SkillResolution = HashMap<String, String>;
...
pub(crate) async fn resolve_skill_dependencies_for_turn(
    ...
    let mut loaded_values = SkillResolution::new();
...
async fn request_skill_dependencies(
    ...
    let questions = dependencies
        .iter()
        .map(|dependency| {
            let skill_name = &dependency.skill_name;
            let name = &dependency.name;
            let requirement = dependency.description.as_ref().map_or_else(
                || format!("The skill \"{skill_name}\" requires \"{name}\" to be set."),
                |description| {
                    format!(
                        "The skill \"{skill_name}\" requires \"{name}\" to be set ({description})."
                    )
                },
            );
            RequestUserInputQuestion {
                ...
                question: format!(
                    "{requirement} This is an experimental internal feature. \
                     The value is stored in memory for this session only."
                ),
                is_other: false,
                is_secret: true,
                options: None,
    ...
    let mut values = SkillResolution::new();
