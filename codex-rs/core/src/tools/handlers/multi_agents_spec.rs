...
pub fn create_followup_task_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        ...
        (
            "model".to_string(),
            JsonSchema::string(Some(
                "Optional model override for the target agent's next turn. Prefer the current or inherited model unless the follow-up task clearly needs a different model.".to_string(),
            )),
        ),
        (
            "reasoning_effort".to_string(),
            JsonSchema::string(Some(
                "Optional reasoning effort override for the target agent's next turn. Prefer the current or higher effort for ambiguous, risky, code-changing, or verification-heavy work.".to_string(),
            )),
        ),
    ]);
...
pub fn create_resume_agent_tool_v2() -> ToolSpec {
    let properties = BTreeMap::from([(
        "target".to_string(),
        JsonSchema::string(Some(
            "Agent id or canonical task name to resume (from spawn_agent).".to_string(),
        )),
    )]);
...
pub fn create_compact_agent_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target".to_string(),
            JsonSchema::string(Some(
                "Agent id or canonical task name to compact (from spawn_agent).".to_string(),
            )),
        ),
        (
            "reason".to_string(),
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description:
            "Request context compaction for a live non-root MultiAgentV2 subagent that is idle, interrupted, or completed."
                .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::object(
...
pub fn create_restart_agent_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target".to_string(),
            JsonSchema::string(Some(
                "Agent id or canonical task name to restart (from spawn_agent).".to_string(),
            )),
        ),
        (
            "message".to_string(),
        ...
        (
            "model".to_string(),
            JsonSchema::string(Some(
                "Optional model override for the restarted agent's next turn. Prefer the current or inherited model unless the restarted task clearly needs a different model.".to_string(),
            )),
        ),
        (
            "reasoning_effort".to_string(),
            JsonSchema::string(Some(
                "Optional reasoning effort override for the restarted agent's next turn. Prefer the current or higher effort for ambiguous, risky, code-changing, or verification-heavy work.".to_string(),
            )),
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description:
            "Restart a non-root MultiAgentV2 subagent by shutting down its live thread tree, resuming it from rollout, and optionally assigning follow-up work."
                .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::object(
...
fn compact_agent_output_schema() -> Value {
    json!({
        ...
        "properties": {
            ...
            "current_status": {
                "description": "The agent status observed after compaction completed.",
                "allOf": [agent_status_output_schema()]
            }
        },
...
fn restart_agent_output_schema() -> Value {
    json!({
        ...
        "properties": {
            "previous_status": {
                "description": "The agent status observed before restart was requested.",
                "allOf": [agent_status_output_schema()]
            },
            "status": {
                "description": "The agent status observed after restart completed.",
                "allOf": [agent_status_output_schema()]
            }
        },
