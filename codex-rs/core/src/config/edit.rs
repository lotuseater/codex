...
struct ConfigDocument {
    doc: DocumentMut,
    /// Fork feature: when set, generic `SetPath`/`ClearPath` edits are written
    /// under `[profiles.<name>]` instead of the document root. Typed setters
    /// (model, notices, mcp_servers, ...) always remain global, matching the
    /// fork's per-edit `Scope` behavior.
...
impl ConfigDocument {
    ...
    /// Prepend `["profiles", <name>]` to an owned segment path when a profile is
    /// active and the path is not already profile-scoped. Used only for the
    /// generic `SetPath`/`ClearPath` edits so profile-targeted config writes land
    /// in the right section; typed setters stay global.
    fn profile_scoped_owned(&self, segments: &[String]) -> Vec<String> {
        if let Some(profile) = self.profile.as_deref()
            && segments.first().is_none_or(|segment| segment != "profiles")
        {
    ...
    fn apply(&mut self, edit: &ConfigEdit) -> anyhow::Result<bool> {
        match edit {
            ...
            ConfigEdit::SetContextBudgetMode { mode } => Ok(self.write_optional_value(
                &["context_budget_mode"],
                Some(value(mode.to_string())),
            )),
            ...
            ConfigEdit::SetPath { segments, value } => {
                let segments = self.profile_scoped_owned(segments);
...
pub fn apply_blocking(codex_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
    ...
    apply_blocking_to_resolved_file(&config_path, edits, None)
}
...
impl ConfigEditsBuilder {
    ...
    /// Fork feature: scope generic `set_path`/`clear_path` edits to
    /// `[profiles.<name>]` in the target config file. Typed setters
    /// (model, notices, ...) remain global regardless of this setting.
    ...
    pub fn set_context_budget_mode(mut self, mode: ContextBudgetMode) -> Self {
        self.edits.push(ConfigEdit::SetContextBudgetMode { mode });
        self
    }
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking_to_resolved_file(&self.config_path, &self.edits, self.profile.as_deref())
    }
