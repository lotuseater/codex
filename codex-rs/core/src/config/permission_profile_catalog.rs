...
// fork-local: upstream defines `is_permission_allowed` in `config::mod`, but the fork's
// split-file layout keeps this catalog self-contained. Body matches upstream verbatim.
fn is_permission_allowed(
    allowed_permission_profiles: &std::collections::BTreeMap<String, bool>,
    profile_id: &str,
) -> bool {
    allowed_permission_profiles
        .get(profile_id)
