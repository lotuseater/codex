...
impl ShellSnapshot {
    ...
    /// Regenerates the shell snapshot for a newly selected cwd.
    ///
    /// Restored (and adapted) from fork parent `b342f16013` (`shell_snapshot.rs:63`). The fork
    /// broadcast each refreshed snapshot through `shell_snapshot_tx`
    /// (`watch::Sender<Option<Arc<ShellSnapshot>>>`) so the active `Shell` could observe it. The
    /// merge replaced that watch-channel design with the per-turn `ThreadEnvironments` snapshot
    /// future (see `environment_selection.rs`), so the channel's receiver is no longer read and its
    /// payload type (`Arc<ShellSnapshot>`, the config) no longer matches a built snapshot
    /// (`Arc<ShellSnapshotFile>`). We keep the fork's side effect — rebuild the snapshot file for the
    /// new cwd in the background — using the current builder, and accept (without writing through)
    /// the now-orphaned sender so the surviving caller keeps compiling under union-preserve.
    pub(crate) fn refresh_snapshot(
        ...
        // The watch channel is a fork remnant with no live reader in the current design; consume the
        // sender to keep the fork call-site signature intact without resurrecting a dead mechanism.
        let _ = shell_snapshot_tx;
        ...
        drop(tokio::spawn(async move {
            let _ = ShellSnapshot::build_for_cwd(config, session_cwd, shell).await;
        }));
    }
