...
impl ModelClient {
    ...
    /// Invalidates the cached websocket session so the next stream opens a fresh one.
    ///
    /// Restored from fork parent `b342f16013` (`client.rs:405`). The fork tracked a numeric
    /// `window_generation` counter on `ModelClientState`; the merge dropped that field and moved
    /// context-window numbering onto `Session` (`auto_compact_window_number`). The only live effect
    /// the surviving caller (`session_history.rs` after a compaction) needs is dropping the cached
    /// websocket session so the next turn reconnects under the advanced window.
    ...
    fn new(
        ...
        Self {
            auth_mode: auth.map(|auth| match auth {
                ...
                // fork-local: upstream added CodexAuth::PersonalAccessToken; map it to the
                // closest RuntimeAuthMode (telemetry groups it under "Chatgpt").
                CodexAuth::PersonalAccessToken(_) => RuntimeAuthMode::Chatgpt,
                // fork-local: upstream added CodexAuth::BedrockApiKey; map it to the closest
                // RuntimeAuthMode (telemetry groups it under "ApiKey").
                CodexAuth::BedrockApiKey(_) => RuntimeAuthMode::ApiKey,
            }),
