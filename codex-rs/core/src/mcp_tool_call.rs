...
fn config_edits_builder_for_config(config: &Config) -> ConfigEditsBuilder {
    // The upstream merge replaced the edit-time `with_profile(<id>)` selector with
    // load-time profile resolution: each profile now persists to its own
    // `<profile>.config.toml`, and the active file is recorded in the config layer
    // stack. `for_config` reads that already-resolved user config file, so MCP-tool
    // approval edits still land in the active permission profile's config file.
