        ...
        if should_clear_active_turn && matches!(finished_task_kind, Some(TaskKind::Regular)) {
            let pending_after_compact: Vec<ResponseInputItem> = self
                ...
                .filter_map(|input| match input {
                    ...
                    // recorded as conversation items when first injected, so they do
                    // not need to be re-queued here; drop them instead of attempting a
                    // lossy reverse conversion. Revisit if the `TurnInput` /
                    // `idle_pending_input` types are reconciled across input_queue.rs +
                    // session_mailbox.rs.
                    //
                    // Fork: `TurnInput::InterAgentCommunication` (multi-agent mailbox)
                    // is likewise already recorded as a conversation item when injected,
                    // so it is dropped here rather than re-queued — matching the handling
                    // in session/turn.rs and tasks/review.rs.
