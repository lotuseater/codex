...
use codex_protocol::config_types::ConfigLayerSource;
...
impl<'a> AgentsMdManager<'a> {
    ...
    /// fork feature: loads global AGENTS.md instructions from the Codex home
    /// directory (independent of any project root).
    ...
    async fn user_instructions_with_fs(
        ...
        match read_agents_md(self.config, fs, LOCAL_ENVIRONMENT_ID, &cwd).await {
        ...
        // fork feature `Feature::ChildAgentsMd`: append hierarchical-AGENTS
        // guidance so child agents understand nested AGENTS.md precedence.
