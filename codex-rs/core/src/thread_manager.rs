...
impl ThreadManager {
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                thread_store,
                live_thread_factory: Arc::new(UnsupportedLiveThreadFactory::new()),
                state_db,
                agent_graph_store,
    ...
    async fn start_thread_with_options_and_fork_source(
        ...
        let (resumed_session_source, resumed_thread_source) = (|| {
            if let InitialHistory::Resumed(resumed) = &options.initial_history {
...
impl ThreadManagerState {
    ...
    // fork-local: restored after upstream replaced the stored `state_db` with a
    // derived `agent_graph_store`; the fork still threads the raw `StateDbHandle`
    // into `CodexSpawnArgs::state_db` for extended-history persistence.
    ...
    async fn effective_originator(
        ...
        let persisted_originator = match initial_history {
            InitialHistory::Resumed(resumed) => resumed.history.iter().find_map(|item| {
                if let RolloutItem::SessionMeta(meta_line) = item {
                    let orig = meta_line.meta.originator.clone();
                    (!orig.is_empty()).then_some(orig)
                } else {
                    None
                }
            }),
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        // fork-local: derive the resumed multi-agent mode from history so it can be
        // threaded into CodexSpawnArgs (upstream dropped the per-spawn mode param).
