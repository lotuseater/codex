...
impl Session {
    ...
    pub(crate) async fn build_initial_context_with_world_state(
        ...
        if let Some(usage_hint_text) = multi_agent_v2_usage_hint_text {
            ...
            // across mid-turn compactions: this rebuild path feeds
            // `build_compaction_initial_context`, so every compaction
            // re-delivers the framing to the replacement history.
    ...
    /// Upstream world-state machinery (snapshot/patch model). Unioned alongside the fork's
    /// `record_step_environment_context_if_changed` so callers of either surface resolve.
...
// ---- folded from session/approvals.rs (merge-7 D-2 fold) ----
impl Session {
...
// ---- folded from session/codex_handle.rs (merge-7 D-2 fold) ----
...
pub(crate) struct CodexSpawnArgs {
    ...
    // fork-local: unioned upstream field (provider/model fallback gate).
    ...
    // fork-local: unioned upstream field (code-mode session provider).
    pub(crate) code_mode_session_provider: Arc<dyn codex_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<codex_extension_api::ExtensionRegistry<crate::config::Config>>,
    pub(crate) conversation_history: InitialHistory,
    // fork-local: unioned upstream field (requested thread history mode).
...
// ---- folded from session/initial_context.rs (merge-7 D-2 fold) ----
impl Session {
...
// ---- folded from session/session_events.rs (merge-7 D-2 fold) ----
...
// ---- folded from session/session_history.rs (merge-7 D-2 fold) ----
impl Session {
...
// ---- folded from session/session_lifecycle.rs (merge-7 D-2 fold) ----
...
// ---- folded from session/session_mailbox.rs (merge-7 D-2 fold) ----
impl Session {
    ...
    pub async fn steer_input(
        ...
        turn_state.push_pending_input(TurnInput::UserInput {
            content: input,
            client_id: None,
        });
...
// ---- folded from session/session_network_proxy.rs (merge-7 D-2 fold) ----
impl Session {
...
// ---- folded from session/session_settings.rs (merge-7 D-2 fold) ----
impl Session {
