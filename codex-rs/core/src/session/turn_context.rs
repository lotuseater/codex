...
pub(crate) struct TurnSkillsContext {
    ...
    // fork-local: convenience handle to the loaded skills outcome carried by the
    // host snapshot, so existing fork readers can keep accessing
    // `turn_skills.outcome` directly (as `&SkillLoadOutcome` via deref).
...
impl TurnSkillsContext {
    pub(crate) fn new(snapshot: HostSkillsSnapshot) -> Self {
        let outcome = Arc::new(snapshot.outcome().clone());
        Self {
            snapshot,
            outcome,
...
pub struct TurnContext {
    ...
    /// fork-local: per-turn override for the auto-compaction summarization
    /// prompt. Restored after the upstream merge dropped it from `TurnContext`;
    ...
    // fork-local: the managed-features view resolved for this turn (mirrors
    // `config.features`); kept as a direct field so existing fork readers keep
    // working alongside readers that go through `config.features`.
    pub features: ManagedFeatures,
    // fork-local: model-derived output truncation policy for this turn.
...
impl TurnContext {
    ...
    /// fork-local: returns the per-turn auto-compaction summarization prompt,
    /// falling back to the built-in [`crate::compact::SUMMARIZATION_PROMPT`].
    /// Restored after the upstream merge dropped it from `TurnContext`.
    ...
    pub(crate) async fn with_model(
        ...
        // fork-local: carry the managed-features view and model-derived truncation
        // policy for this turn (mirrors `config.features`).
...
impl Session {
    ...
    pub(crate) fn build_per_turn_config(
        ...
        per_turn_config.context_budget_mode = session_configuration.context_budget_mode;
    ...
    pub(crate) fn make_turn_context(
        ...
        cwd: AbsolutePathBuf,
        sub_id: String,
        // fork-local: goals feature — ephemeral suppression flag for goal tools.
        ...
        // fork-local: carry the managed-features view and model-derived truncation
        // policy for this turn (mirrors `per_turn_config.features`).
        let features = per_turn_config.features.clone();
        let truncation_policy = model_info.truncation_policy.into();
        // Capture before `config: per_turn_config` moves the Arc into the literal.
        let shell_environment_policy = per_turn_config.permissions.shell_environment_policy.clone();
        TurnContext {
            sub_id,
    ...
    async fn new_turn_context_from_configuration(
        ...
        let skills_snapshot = self
            ...
            .await;
        // fork-local: Goal tools are hidden for ephemeral threads (e.g. review
        // subagents). The goal-tools feature gate itself is applied inside
        // `ToolsConfig`; this flag only carries the ephemeral suppression,
        // mirroring the `!config.ephemeral` gate used for review threads.
