...
pub(crate) fn guardian_agent_spawner(
    ...
    move |forked_from_thread_id: ExtensionThreadId,
        ...
        Box::pin(async move {
            let forked_from_thread_id = codex_protocol::ThreadId::from_string(
                forked_from_thread_id.as_str(),
            )
            .map_err(|err| {
                CodexErr::UnsupportedOperation(format!("invalid fork source thread id: {err}"))
            })?;
