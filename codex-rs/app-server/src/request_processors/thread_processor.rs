...
fn validate_dynamic_tools(tools: &[ApiDynamicToolSpec]) -> Result<(), String> {
    ...
    for tool in tools {
        ...
        if let Err(err) = codex_tool_schema::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn memory_status(
        ...
        self.memory_status_response_inner()
            .await
            .map(|response: MemoryStatusResponse| Some(response.into()))
    }
    ...
    fn build_thread_config_overrides(
        ...
        ConfigOverrides {
            ...
            // Upstream `runtimeWorkspaceRoots` wired through to the cwd-derived
            // workspace-root override. The merged `ConfigOverrides.workspace_roots`
            // field is `Vec<PathBuf>`, so convert from `AbsolutePathBuf` here.
            workspace_roots: runtime_workspace_roots.map(|roots| {
                roots
                    .into_iter()
                    .map(AbsolutePathBuf::into_path_buf)
                    .collect()
    ...
    async fn memory_status_response_inner(
        &self,
    ) -> Result<MemoryStatusResponse, JSONRPCErrorError> {
        ...
        let mut response = MemoryStatusResponse {
            ...
            state_db_available: self.state_db.is_some(),
        ...
        let snapshot = state_db.memory_status_snapshot().await.map_err(|err| {
            internal_error(format!("failed to read memory status from state db: {err}"))
        })?;
