...
// fork-local: fork drives managed-config test isolation through CLI args and a
// ManagedConfigTestArg selector (see new_with_program_env_and_args below).
pub const DISABLE_MANAGED_CONFIG_ARG: &str = "--disable-managed-config-for-tests";
pub const MANAGED_CONFIG_PATH_ARG: &str = "--managed-config-path-for-tests";

enum ManagedConfigTestArg {
    DefaultPath,
    Disabled,
    Path(PathBuf),
}
...
impl TestAppServer {
    ...
    // fork-local: retained convenience constructor; upstream now prefers
    // TestAppServer::builder(), but fork tests still call `new`.
    pub async fn new(codex_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_args_and_managed_config(
            codex_home,
            &[],
    ...
    pub async fn new_with_managed_config_path(
        codex_home: &Path,
        ...
        Self::new_with_env_args_and_managed_config(
            ...
            &[],
            &[DISABLE_PLUGIN_STARTUP_TASKS_ARG],
            ManagedConfigTestArg::Path(managed_config_path),
        )
        .await
    }
    ...
    async fn new_with_program_env_and_args(
        ...
        match managed_config {
            ManagedConfigTestArg::DefaultPath => {
                cmd.arg(MANAGED_CONFIG_PATH_ARG)
                    .arg(codex_home.join("managed_config.toml"));
            }
            ManagedConfigTestArg::Disabled => {
                cmd.arg(DISABLE_MANAGED_CONFIG_ARG);
            }
            ManagedConfigTestArg::Path(path) => {
                cmd.arg(MANAGED_CONFIG_PATH_ARG).arg(path);
            }
        }
...
impl TestAppServerBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        // fork-local: upstream's builder predates the fork's ManagedConfigTestArg
        // selector on new_with_program_env_and_args; pass the fork default so the
        // union compiles. (The arg-vs-env managed-config reconciliation is handled
        // by the test-repair wave — see FILES_UNCERTAIN note for this file.)
