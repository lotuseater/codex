//! Domain-neutral tool configuration types shared by core and tool adapters.

use crate::ShellCommandBackendConfig;
use crate::UnifiedExecShellMode;
use crate::ZshForkConfig;
...
pub(crate) fn request_user_input_available_modes(features: &Features) -> Vec<ModeKind> {
...
fn unified_exec_shell_mode_for_session(
    ...
    if cfg!(unix)
        ...
        && let (Ok(shell_zsh_path), Ok(main_execve_wrapper_exe)) = (
            AbsolutePathBuf::try_from(shell_zsh_path.as_path()).inspect_err(|err| {
                tracing::warn!("Failed to convert shell_zsh_path `{shell_zsh_path:?}`: {err:?}")
            }),
            AbsolutePathBuf::try_from(main_execve_wrapper_exe.as_path()).inspect_err(|err| {
                tracing::warn!(
    ...
    {
        UnifiedExecShellMode::ZshFork(ZshForkConfig::new(
            shell_zsh_path,
...
impl ToolsConfig {
    ...
    pub fn with_unified_exec_shell_mode_for_session(
        ...
        self.unified_exec_shell_mode = unified_exec_shell_mode_for_session(
...
impl codex_tool_registry_api::McpToolExposureConfig for ToolsConfig {
