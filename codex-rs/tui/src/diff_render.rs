...
// fork-local: `impl Renderable for FileChange` lives in `codex_tui_render::diff_render`
// (both `Renderable` and `FileChange` are owned by that crate now). A duplicate impl here
// would violate the orphan rule (E0117); the foreign impl already covers the
// `Box::new(row.change) as Box<dyn Renderable>` coercion below.
