//! Mention/skill/plugin token logic for [`ChatComposer`]: detecting the active
//! @-token, inserting selected paths/mentions, and snapshotting mention bindings.
use super::*;
...
impl ChatComposer {
    ...
    pub(crate) fn current_mentions_v2_token(&self) -> Option<String> {
    ...
    pub(crate) fn insert_selected_mention(&mut self, insert_text: &str, path: Option<&str>) {
        let cursor_offset = self.textarea.cursor();
        let text = self.textarea.text();
        let safe_cursor = Self::clamp_to_char_boundary(text, cursor_offset);
