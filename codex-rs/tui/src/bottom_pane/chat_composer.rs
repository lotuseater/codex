...
pub(crate) struct ChatComposer {
    ...
    // fork-local: realtime voice conversation / audio-device selection gating + session-limit
    // status line. Preserved as composer-owned state because external callers
    // (`bottom_pane::config` / `bottom_pane::mod`) drive these setters.
    realtime_conversation_enabled: bool,
    audio_device_selection_enabled: bool,
    session_limit_status_line: Option<Line<'static>>,
    // fork-local: inline audio recording-meter placeholders. Re-applied on the upstream SPLIT
    // baseline over the payload-keyed textarea element API (the fork's named-element-by-id textarea
    // helpers were removed by the upstream textarea adoption). Maps a stable synthetic id to the
    // element's current payload text so updates/removes can address the right element.
    #[cfg(not(target_os = "linux"))]
    recording_meter_payloads: HashMap<String, String>,
    #[cfg(not(target_os = "linux"))]
    recording_meter_next_id: u64,
...
impl ChatComposer {
    ...
    pub(crate) fn new_with_config(
        ...
        let mut this = Self {
            ...
            #[cfg(not(target_os = "linux"))]
            recording_meter_payloads: HashMap::new(),
            #[cfg(not(target_os = "linux"))]
            recording_meter_next_id: 0,
            history_search: None,
    ...
    // fork-local: realtime voice / audio-device selection gating. Stored composer state so the
    // ChatWidget feature toggles (`bottom_pane::config`) keep compiling.
    ...
    // fork-local: session-limit status line surfaced in the footer-right slot. Returns whether the
    // value changed so the caller (`bottom_pane::mod`) can gate a redraw.
    ...
    // fork-local: inline audio recording-meter placeholders, re-applied on the SPLIT baseline.
    //
    // The fork drove these through a named-element-by-string-id textarea API that the upstream
    // textarea adoption removed; here they are rebuilt over the payload-keyed element API
    // (`insert_element` / `replace_element_payload`) using a composer-owned id -> payload map so
    // the external `BottomPane` wrappers keep a stable id across updates.
    #[cfg(not(target_os = "linux"))]
    pub fn insert_recording_meter_placeholder(&mut self, text: &str) -> String {
        let id = self.recording_meter_next_id;
        self.recording_meter_next_id = self.recording_meter_next_id.wrapping_add(1);
        let id = id.to_string();
        self.draft.textarea.insert_element(text);
        self.recording_meter_payloads
    ...
    pub fn update_recording_meter_in_place(&mut self, id: &str, text: &str) -> bool {
        let Some(current) = self.recording_meter_payloads.get(id).cloned() else {
            return false;
        };
        if current == text {
            return false;
        }
        let updated = self.draft.textarea.replace_element_payload(&current, text);
        if updated {
            self.recording_meter_payloads
    ...
    pub fn remove_recording_meter_placeholder(&mut self, id: &str) {
        if let Some(current) = self.recording_meter_payloads.remove(id) {
            let _ = self.draft.textarea.replace_element_payload(&current, "");
        }
    }
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                // fork-local: `ConnectorsSnapshot` carries `codex_app_server_protocol::AppInfo`,
                // but `connector_{display_label,mention_slug}` now take `codex_app_catalog_types::AppInfo`.
                // Both helpers only read `.name`, so derive the label/slug from the name directly to
                // avoid a cross-crate `AppInfo` conversion while keeping the fork's snapshot type.
                let display_name = connector.name.clone();
                let description = Some(Self::connector_brief_description(connector));
                let slug =
                    codex_connectors::metadata::connector_mention_slug_from_name(&connector.name);
